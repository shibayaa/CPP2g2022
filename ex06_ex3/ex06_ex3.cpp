...
public:
	...
	virtual double GetPackageLength() = 0;	//追加
};
...
int GetBoxKuronekoPackage(Box& box) {
	...
	int packageSize = 0;
	// 箱の宅急便のサイズを求める
	double length = box.GetPackageLength();
...
int GetCylinderKuronekoPackage(Cylinder& cylinder) {
	...
	int packageSize = 0;
	// 箱の宅急便のサイズを求める
	double length = cylinder.GetPackageLength();
	for (i = 0; i < _countof(kuronekoSize); i++) {
		if (length <= kuronekoSize[i]) {
			packageSize = kuronekoSize[i];
			break;
...
int GetConeKuronekoPackage(Cone& cone) {
	...
	int packageSize = 0;
	// 箱の宅急便のサイズを求める
	double length = cone.GetPackageLength();
	for (i = 0; i < _countof(kuronekoSize); i++) {
		if (length <= kuronekoSize[i]) {
			packageSize = kuronekoSize[i];
			break;
...
int GetSphereKuronekoPackage(Sphere& sphere) {
	...
	int packageSize = 0;
	// 箱の宅急便のサイズを求める
	double length = sphere.GetPackageLength();
	for (i = 0; i < _countof(kuronekoSize); i++) {
		if (length <= kuronekoSize[i]) {
			packageSize = kuronekoSize[i];
			break;
